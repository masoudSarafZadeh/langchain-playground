...
def read_todos(
    ...
    for i, todo in enumerate(todos, 1):
        ...
        # Defensive extraction: Handles cases where Todo is defined as either a TypedDict or a Pydantic model
        status = todo.get("status", "pending") if isinstance(todo, dict) else getattr(todo, "status", "pending")
        content = todo.get("content", "") if isinstance(todo, dict) else getattr(todo, "content", "")
